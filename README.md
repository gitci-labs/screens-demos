...
- `demos/reader-immersive`: a reading/study app scene set with target-specific feature closeups, English/Japanese localized copy, target-pattern variant groups for PPO-style experiments, and Fastlane export coverage.
...
The reader demo exercises localized scene sets:

```sh
GITCI_SCREENS_JS_WORKSPACE="$PWD/screens/js" \
GITCI_SCREENS_TEMPLATES_ROOT="$PWD/screens/templates/gitci/screens" \
screens/.build/debug/gitci-screens export screens-demos/demos/reader-immersive --strict --fastlane-out fastlane/screenshots
```

To render every declared reader experiment with separate default build folders:
